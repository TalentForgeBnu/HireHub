...
<body>
	...
	<c:if test="${dossie == null}">
		<form action="<%=request.getContextPath()%>/inserir-dossie"
			method="post">
	</c:if>
	<c:if test="${dossie != null}">
		<form action="<%=request.getContextPath()%>/atualizar-dossie"
			method="post">
	</c:if>
	<div>
		<h2>Dossie</h2>
		<input type="hidden" name="id-aluno" value="${aluno.id}"> <input
			type="hidden" name="aluno-cpf" value="${aluno.cpf}"> <input
			type=hidden name="id-dossie" value="${dossie.id}">
		<textarea placeholder="Escreva o Dossiê" id="conteudo" name="conteudo">${dossie.conteudo}</textarea>
		<input type="submit" value="Cadastrar">
	</div>
	</form>

	<form action="<%=request.getContextPath()%>/inserir-apontamento"
		method="post">
		<div>
			<h2>Apontamento</h2>
			<input type="hidden" name="id-aluno" value="${aluno.id}"> <input
				type="hidden" name="aluno-cpf" value="${aluno.cpf}"> <input
				type=hidden name="id-dossie" value="${dossie.id}">
			<textarea placeholder="Escreva o Apontamento" name="apontamento"
				id="apontamento"></textarea>
			<div>
