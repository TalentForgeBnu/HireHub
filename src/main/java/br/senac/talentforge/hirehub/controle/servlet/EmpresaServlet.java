...
public class EmpresaServlet extends HttpServlet {
		...
		if (empresa.equals(session.getAttribute("usuario-logado"))) {
			...
			if (empresa != null) {
				...
				usuarioDAO.inserirUsuario(
						new Empresa(senha, endereco, papel, telefone, email, cnpj, nome, dataFundacao, descricao));
			}
		}
	}
	...
	private void atualizarPerfilEmpresa(HttpServletRequest request, HttpServletResponse response)
		...
		empresaRecuperada = (Empresa) session.getAttribute("usuario-logado");

		if (empresaRecuperada.equals(session.getAttribute("usuario-logado"))) {
			...
			if (empresaRecuperada != null) {
				...
				Endereco endereco = new Endereco(logradouro, bairro, cidade, estado, cep, 1 /* numero */, complemento,
						via);
	...
	private void recuperarPerfilEmpresa(HttpServletRequest request, HttpServletResponse response)
		...
		HttpSession session = request.getSession();
		Empresa empresa = (Empresa) session.getAttribute("usuario-logado");
